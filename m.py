...
async def start(update: Update, context: CallbackContext):
    ...
    if user_id == ADMIN_USER_ID or is_secondary_admin(user_id):  # ✅ Bypass for secondary admin
        help_text = (
            "*ℹ️ Admin Help Menu*\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id == ADMIN_USER_ID or is_secondary_admin(user_id):  # ✅ Bypass for secondary admin
        ...
        cooldown_time = 600  # ✅ No cooldown for secondary admin  # Cooldown period for regular users
    ...
    if not (user_id == ADMIN_USER_ID or is_secondary_admin(user_id)) and duration > 240:  # ✅ Secondary admin allowed full duration
        await context.bot.send_message(
            chat_id=chat_id,
...
async def add_friend(update: Update, context: CallbackContext):
    ...
    if len(args) != 2:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*⚠️ Usage: /add <user_id> <expiry_time>*\nExamples: 1d, 1m",
            parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="*❌ Invalid user ID. It must be a number.*",
            parse_mode="Markdown"
        )
        return
    ...
    if not match:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*❌ Invalid time format. Use like 30mi, 2h, 1d, 1m*",
            parse_mode="Markdown"
        )
        return
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"*✅ User {friend_id} granted access until {expiry.strftime('%Y-%m-%d %H:%M:%S')} UTC.*",
        parse_mode="Markdown"
    )
...
async def post(update: Update, context: CallbackContext):
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text("⚠️ Please reply to a media message (photo, video, document, audio, voice, or text) with /post.")
        return
...
async def add_admin(update: Update, context: CallbackContext):
    if update.effective_user.id != ADMIN_USER_ID:
        await update.message.reply_text("❌ Tum authorized nahi ho.")
        return
    if len(context.args) != 2:
        await update.message.reply_text("🧾 Usage: /add_admin <user_id> <days>")
        return
    try:
        ...
        await update.message.reply_text(f"✅ User {user_id} secondary admin bana diya gaya hai {days} din ke liye.")
    except:
        await update.message.reply_text("❌ Galat format diya gaya hai.")
...
async def remove_admin(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ User {user_id} ko secondary admin se hata diya gaya.")
    except:
